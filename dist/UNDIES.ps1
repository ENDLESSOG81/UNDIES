param(
    ...
    [ValidateSet('initialize','doctor','status','session-start','session-close','blue-report','resume','adopt','configure','version','upgrade','integrity','repair','rollback','disable','remove','help')]
    [string]$Command = 'help',
    [string]$SessionId,
...
function Invoke-PortableAdoption($Root){ $inventory=@(Get-PortableInventory $Root); $conflicts=@(); if(Test-Path (Join-Path $Root '.undies/config/project.json')){$conflicts += 'Existing UNDIES manifest'}; $proposal=[ordered]@{ mode=if($DryRun){'DRY_RUN'}elseif($Preview){'PREVIEW'}elseif($Confirm){'APPLY'}else{'PREVIEW'}; project_name=if($ProjectName){$ProjectName}else{Split-Path -Leaf $Root}; project_code=if($ProjectCode){$ProjectCode}else{'UND'}; existing_file_count=$inventory.Count; conflicts=$conflicts; files_to_create=@('.undies/config/project.json','.undies/adoption/baseline.json','.undies/recovery/rollback-manifest.json'); untouched_files=@($inventory.path); git=(Get-PortableGitInfo $Root); recommended_gitignore=@('.undies/','*.log','.env','.env.*'); proposed_git_actions=@('No automatic git add, commit, push, pull, merge, reset, checkout, restore, clean, stash, or branch changes') }
    ...
    $manifestData=Initialize-Portable $Root
    $manifestData.project_name=$proposal.project_name
    $manifestData.project_code=$proposal.project_code
    $manifestData.updated_date=Get-UndiesUtcTime
    Save-Json (Join-Path $Root '.undies/config/project.json') $manifestData
    New-Item -ItemType Directory -Force -Path (Join-Path $Root '.undies/adoption')|Out-Null; Save-Json (Join-Path $Root '.undies/adoption/baseline.json') ([ordered]@{created_utc=Get-UndiesUtcTime; inventory=$inventory; git=$proposal.git}); Save-Json (Join-Path $Root '.undies/recovery/rollback-manifest.json') ([ordered]@{created_utc=Get-UndiesUtcTime; operation='adoption'; managed_files=$proposal.files_to_create; host_files_preserved=@($inventory.path)}); return $proposal }
function Invoke-PortableDisable($Root){ Initialize-Portable $Root|Out-Null; $state=[ordered]@{disabled=$true;disabled_utc=Get-UndiesUtcTime;reversible=$true}; Save-Json (Join-Path $Root '.undies/config/disabled.json') $state; $state }
function Invoke-PortableRollback($Root){ $manifest=Join-Path $Root '.undies/recovery/rollback-manifest.json'; $plan=[ordered]@{status='GREEN';operation='ROLLBACK';manifest_exists=(Test-Path $manifest);preview=(!$Apply);actions=@('Restore UNDIES-managed files to recorded checkpoint','Preserve host files','Preserve rollback evidence')}; if($Preview -or -not $Apply){return $plan}; Initialize-Portable $Root|Out-Null; Save-Json (Join-Path $Root '.undies/reports/rollback-report.json') ([ordered]@{status='GREEN';applied_utc=Get-UndiesUtcTime;manifest_exists=$plan.manifest_exists}); return [pscustomobject]@{status='GREEN';operation='ROLLBACK';validated=$true} }
function Invoke-PortableRemove($Root){ $managed=@('.undies/config','.undies/runtime','.undies/sessions','.undies/recovery','.undies/governance'); $plan=[ordered]@{status='GREEN';operation='REMOVE';preview=(!$Apply);managed_files=$managed;preserve=@('UNDIES.ps1','.git','.undies/reports','.undies/evidence')}; if($Preview -or -not $Apply){return $plan}; New-Item -ItemType Directory -Force -Path (Join-Path $Root '.undies/reports')|Out-Null; Save-Json (Join-Path $Root '.undies/reports/removal-manifest.json') ([ordered]@{removed=$managed;preserved=$plan.preserve;removed_utc=Get-UndiesUtcTime}); foreach($m in $managed){$path=Join-Path $Root $m; if(Test-Path $path){Remove-Item -LiteralPath $path -Recurse -Force}}; return [pscustomobject]@{status='GREEN';operation='REMOVE';removed=$managed;preserved=$plan.preserve} }
...
function Invoke-PortableVersion($Root){ $manifest=Join-Path $Root '.undies/config/project.json'; $installed=if(Test-Path $manifest){(Read-Json $manifest).version}else{'NONE'}; [pscustomobject]@{portable_version='0.2.0-alpha.1';installed_version=$installed;schema_version='0.1.0'} }
function Invoke-PortableUpgrade($Root){ Initialize-Portable $Root|Out-Null; $manifest=Join-Path $Root '.undies/config/project.json'; $m=Read-Json $manifest; $cmp=Compare-VersionText $m.version '0.2.0-alpha.1'; if($cmp -gt 0){ return [pscustomobject]@{status='BLUE';reason='Installed UNDIES version is newer than portable bootstrap';installed_version=$m.version;portable_version='0.2.0-alpha.1';manual_action='Use a matching or newer UNDIES bootstrap';validation_command='.\UNDIES.ps1 version';resume_checkpoint='upgrade-version-compatible'} }
    $plan=[ordered]@{status='GREEN';operation=if($m.version -ne '0.2.0-alpha.1'){'UPGRADE'}else{'SAME_VERSION'};installed_version=$m.version;portable_version='0.2.0-alpha.1';legacy_status_aliases=@{WAITING_FOR_EXTERNAL_DEPENDENCY='BLUE'};will_backup=@('.undies/config','.undies/sessions','.undies/evidence','.undies/reports','.undies/recovery')}
    if($Check -or $Preview -or -not $Apply){ return $plan }
    $backup=Join-Path $Root ('.undies/upgrade/backups/' + (Get-Date -Format yyyyMMddHHmmss)); New-Item -ItemType Directory -Force -Path $backup|Out-Null; Copy-Item -LiteralPath (Join-Path $Root '.undies/config') -Destination $backup -Recurse -Force; $m.version='0.2.0-alpha.1'; $m.updated_date=Get-UndiesUtcTime; Save-Json $manifest $m; $report=[ordered]@{status='GREEN';operation=$plan.operation;backup=$backup;validated=$true}; Save-Json (Join-Path $Root '.undies/reports/upgrade-report.json') $report; return $report }
...
function New-PortableSession($Root){ $project=Initialize-Portable $Root; $dir=Join-Path $Root '.undies/sessions'; $code=if($project.project_code){$project.project_code}else{'UND'}; $name=if($project.project_name){$project.project_name}else{Split-Path -Leaf $Root}; $version=if($project.version){$project.version}else{'0.2.0-alpha.1'}; $id='UND-'+$code+'-'+(Get-Date -Format yyyyMMdd)+'-'+('{0:000}' -f ((@(Get-ChildItem $dir -Filter '*.json' -ErrorAction SilentlyContinue).Count)+1)); $s=[ordered]@{session_id=$id;project_name=$name;project_code=$code;project_version=$version;workspace=(Resolve-Path $Root).Path;start_time_local=Get-UndiesCentralTime;start_time_utc=Get-UndiesUtcTime;end_time_local=$null;end_time_utc=$null;current_module=$null;completed_modules=@();pending_modules=@();failed_module=$null;warnings=@();resume_point='session-started';final_status='IN_PROGRESS';status='IN_PROGRESS'}; Save-Json (Join-Path $dir "$id.json") $s; $s }
...
switch($Command){
 'help' { 'UNDIES portable bootstrap 0.2.0-alpha.1. BLUE is a safe pause, distinct from BLOCKED and RED. WAITING_FOR_EXTERNAL_DEPENDENCY normalizes to BLUE.' }
 'initialize' { Initialize-Portable $Root | ConvertTo-Json -Depth 20 }
 'doctor' { Initialize-Portable $Root|Out-Null; if($Detailed){ Test-PortableIntegrity $Root | ConvertTo-Json -Depth 20 } else { @{status='GREEN';version='0.2.0-alpha.1';blue='supported';legacy_alias='WAITING_FOR_EXTERNAL_DEPENDENCY=>BLUE';workspace=(Resolve-Path $Root).Path;portable=$true} | ConvertTo-Json -Depth 10 } }
