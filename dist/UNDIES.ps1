...
function Invoke-PortableVersion($Root){ $manifest=Join-Path $Root '.undies/config/project.json'; $installed=if(Test-Path $manifest){(Read-Json $manifest).version}else{'NONE'}; [pscustomobject]@{portable_version='0.2.0-alpha.2';installed_version=$installed;schema_version='0.1.0'} }
function Invoke-PortableUpgrade($Root){ Initialize-Portable $Root|Out-Null; $manifest=Join-Path $Root '.undies/config/project.json'; $m=Read-Json $manifest; $cmp=Compare-VersionText $m.version '0.2.0-alpha.2'; if($cmp -gt 0){ return [pscustomobject]@{status='BLUE';reason='Installed UNDIES version is newer than portable bootstrap';installed_version=$m.version;portable_version='0.2.0-alpha.2';manual_action='Use a matching or newer UNDIES bootstrap';validation_command='.\UNDIES.ps1 version';resume_checkpoint='upgrade-version-compatible'} }
    $plan=[ordered]@{status='GREEN';operation=if($m.version -ne '0.2.0-alpha.2'){'UPGRADE'}else{'SAME_VERSION'};installed_version=$m.version;portable_version='0.2.0-alpha.2';legacy_status_aliases=@{WAITING_FOR_EXTERNAL_DEPENDENCY='BLUE'};will_backup=@('.undies/config','.undies/sessions','.undies/evidence','.undies/reports','.undies/recovery')}
    if($Check -or $Preview -or -not $Apply){ return $plan }
    $backup=Join-Path $Root ('.undies/upgrade/backups/' + (Get-Date -Format yyyyMMddHHmmss)); New-Item -ItemType Directory -Force -Path $backup|Out-Null; Copy-Item -LiteralPath (Join-Path $Root '.undies/config') -Destination $backup -Recurse -Force; $m.version='0.2.0-alpha.2'; $m.updated_date=Get-UndiesUtcTime; Save-Json $manifest $m; $report=[ordered]@{status='GREEN';operation=$plan.operation;backup=$backup;validated=$true}; Save-Json (Join-Path $Root '.undies/reports/upgrade-report.json') $report; return $report }
...
switch($Command){
 'help' { 'UNDIES portable bootstrap 0.2.0-alpha.2. BLUE is a safe pause, distinct from BLOCKED and RED. WAITING_FOR_EXTERNAL_DEPENDENCY normalizes to BLUE.' }
 'initialize' { Initialize-Portable $Root | ConvertTo-Json -Depth 20 }
 'doctor' { Initialize-Portable $Root|Out-Null; if($Detailed){ Test-PortableIntegrity $Root | ConvertTo-Json -Depth 20 } else { @{status='GREEN';version='0.2.0-alpha.2';blue='supported';legacy_alias='WAITING_FOR_EXTERNAL_DEPENDENCY=>BLUE';workspace=(Resolve-Path $Root).Path;portable=$true} | ConvertTo-Json -Depth 10 } }
