...
try {
    ...
    Assert ($manifest.version -eq '0.3.0-alpha.2' -and $manifest.sha256_checksum -eq $hash1) 'manifest invalid'
    ...
    Assert ($init.version -eq '0.3.0-alpha.2' -and $doctor.status -eq 'GREEN') 'empty initialization failed'
    ...
    Assert ($upgrade.status -eq 'GREEN' -and $version.installed_version -eq '0.3.0-alpha.2') 'upgrade failed'
