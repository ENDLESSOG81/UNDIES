...
try {
    ...
    Assert ($version -eq '0.3.0-alpha.1') '01 VERSION mismatch'
    $help=& (Join-Path $RepoRoot 'dist/UNDIES.ps1') help
    Assert ($help -match '0.3.0-alpha.1') '02 packaged bootstrap version mismatch'
    $manifest=Get-Content -LiteralPath (Join-Path $RepoRoot 'dist/RELEASE-MANIFEST.json') -Raw | ConvertFrom-Json
    Assert ($manifest.version -eq '0.3.0-alpha.1') '03 manifest version mismatch'
    ...
    Assert ((Test-Path (Join-Path $Existing '.undies/core/0.3.0-alpha.1/UNDIES.core.ps1'))) '16 versioned core missing'
    ...
    $coreManifest=Get-Content (Join-Path $Existing '.undies/core/0.3.0-alpha.1/CORE-MANIFEST.json') -Raw | ConvertFrom-Json
    ...
    Assert (-not(Test-Path (Join-Path $Existing '.undies/core/0.3.0-alpha.1/project.json'))) '31 project config inside core'
    'runtime'|Set-Content -LiteralPath (Join-Path $Existing '.undies/runtime/state.json') -NoNewline
    Assert (-not(Test-Path (Join-Path $Existing '.undies/core/0.3.0-alpha.1/state.json'))) '32 runtime inside core'
    'extension'|Set-Content -LiteralPath (Join-Path $Existing '.undies/extensions/ext.ps1') -NoNewline
    Assert (-not(Test-Path (Join-Path $Existing '.undies/core/0.3.0-alpha.1/ext.ps1'))) '33 extension inside core'
    Set-ItemProperty -LiteralPath (Join-Path $Existing '.undies/core/0.3.0-alpha.1/UNDIES.core.ps1') -Name IsReadOnly -Value $false -ErrorAction SilentlyContinue
    'tamper'|Add-Content -LiteralPath (Join-Path $Existing '.undies/core/0.3.0-alpha.1/UNDIES.core.ps1')
    ...
    Set-ItemProperty -LiteralPath (Join-Path $Upgrade '.undies/core/0.3.0-alpha.1/UNDIES.core.ps1') -Name IsReadOnly -Value $false -ErrorAction SilentlyContinue
    'collision'|Set-Content -LiteralPath (Join-Path $Upgrade '.undies/core/0.3.0-alpha.1/UNDIES.core.ps1') -NoNewline
    @{active_core_version='0.2.0-alpha.2';active_core_path='.undies/core/0.2.0-alpha.2'}|ConvertTo-Json|Set-Content (Join-Path $Upgrade '.undies/active-version.json') -Encoding UTF8
    Install-ReleaseFiles $Upgrade
