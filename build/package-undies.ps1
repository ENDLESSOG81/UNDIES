...
function Get-PortableInventory(`$Root){ Get-ChildItem -LiteralPath `$Root -Force -Recurse -File | Where-Object { `$_.FullName -notmatch '\\.undies\\' -and `$_.Name -ne 'UNDIES.ps1' } | ForEach-Object { [pscustomobject]@{ path=`$_.FullName.Substring(`$Root.Length).TrimStart('\'); length=`$_.Length; sha256=(Get-FileHash -LiteralPath `$_.FullName -Algorithm SHA256).Hash } } }
function Invoke-PortableAdoption(`$Root){ `$inventory=@(Get-PortableInventory `$Root); `$conflicts=@(); if(Test-Path (Join-Path `$Root '.undies/config/project.json')){`$conflicts += 'Existing UNDIES manifest'}; `$proposal=[ordered]@{ mode=if(`$DryRun){'DRY_RUN'}elseif(`$Preview){'PREVIEW'}elseif(`$Confirm){'APPLY'}else{'PREVIEW'}; project_name=if(`$ProjectName){`$ProjectName}else{Split-Path -Leaf `$Root}; project_code=if(`$ProjectCode){`$ProjectCode}else{'UND'}; existing_file_count=`$inventory.Count; conflicts=`$conflicts; files_to_create=@('.undies/config/project.json','.undies/adoption/baseline.json','.undies/recovery/rollback-manifest.json'); untouched_files=@(`$inventory.path) }
    if(`$DryRun -or `$Preview -or -not `$Confirm){ return `$proposal }
    Initialize-Portable `$Root|Out-Null; New-Item -ItemType Directory -Force -Path (Join-Path `$Root '.undies/adoption')|Out-Null; Save-Json (Join-Path `$Root '.undies/adoption/baseline.json') ([ordered]@{created_utc=Get-UndiesUtcTime; inventory=`$inventory}); Save-Json (Join-Path `$Root '.undies/recovery/rollback-manifest.json') ([ordered]@{created_utc=Get-UndiesUtcTime; operation='adoption'; managed_files=`$proposal.files_to_create; host_files_preserved=@(`$inventory.path)}); return `$proposal }
